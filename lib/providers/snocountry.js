...
function fetch(resort) {
  return new Promise((resolve, reject) => {
    // Check snowApiConfig first (for resorts using snocountry as snowApiProvider)
    // Fall back to apiConfig (for resorts using snocountry as primary apiProvider)
    const config = resort.snowApiConfig || resort.apiConfig;
    if (!config) {
      reject(new Error(`No snowApiConfig or apiConfig for resort: ${resort.key}`));
      return;
    }

    const { resortId } = config;
    if (!resortId) {
