...
function mapDifficulty(diff) {
  // Normalize to app's expected format: Green | Blue | Black | DoubleBlack | TerrainPark
  const map = {
    'beginner': 'Green',
    'novice': 'Green',
    'easy': 'Green',
  ...
  return map[diff?.toLowerCase()] || diff || 'Green';
}
...
const STATUS_ICONS = {
  '169': 'Open',      // Green checkmark
  '94': 'Closed',     // Red X
  '205': 'Groomed',   // Green checkmark with "GRMN" - means Open AND Groomed
  '98': 'Scheduled'   // Scheduled to open
};

// Icons that indicate the trail is groomed
const GROOMED_ICONS = ['205'];
...
async function scrapeSunshineVillage() {
  ...
  while ((match = trailRegex.exec(html)) !== null) {
    ...
    // 205 = Groomed icon (green checkmark with "GRMN") - means Open AND Groomed
    const isGroomed = GROOMED_ICONS.includes(statusCode);
    // Trail is open if status is 169 (Open) or 205 (Groomed - which implies open)
    const isOpen = statusCode === '169' || statusCode === '205';

    trails.push({
      Name: name,
      Status: isGroomed ? 'Open' : (STATUS_ICONS[statusCode] || 'Unknown'),
      IsOpen: isOpen,
      IsGroomed: isGroomed,
      Difficulty: mapDifficulty(DIFFICULTY_ICONS[diffCode])
    });
  }
...
async function scrapeNorquay() {
  ...
  // Parse current temperature - format: "2<span class="forecast__today__weather__degrees">°C</span>"
  // Also try other patterns like ">2°C<" or "2 °C"
  const tempMatch = html.match(/Current Temp[\s\S]*?>(-?\d+(?:\.\d+)?)\s*<span[^>]*>°C/i) ||
                    html.match(/font-weight:\s*700[^>]*>(-?\d+(?:\.\d+)?)\s*<span[^>]*>°C/i) ||
                    html.match(/>\s*(-?\d+(?:\.\d+)?)\s*<span[^>]*forecast__today__weather__degrees[^>]*>°C/i) ||
                    html.match(/(-?\d+(?:\.\d+)?)\s*°\s*C(?:elsius)?/i);
  const temperature_c = tempMatch ? toNumber(tempMatch[1]) : null;

  // Parse forecast data - format: <span class="day-title">Wednesday</span> ... temp-high ... temp-low
  const forecast = [];
  const dayRegex = /<span class="day-title">(\w+)<\/span>[\s\S]*?<div class="temp-high">[\s\S]*?<span class="temp-num">(-?\d+(?:\.\d+)?)[\s\S]*?<span class="temp-low">[\s\S]*?<span class="temp-num">(-?\d+(?:\.\d+)?)/gi;
  let dayMatch;
  while ((dayMatch = dayRegex.exec(html)) !== null) {
    const high_c = toNumber(dayMatch[2]);
    const low_c = toNumber(dayMatch[3]);
    forecast.push({
      day: dayMatch[1],
      high_c,
      low_c,
      high_f: celsiusToFahrenheit(high_c),
      low_f: celsiusToFahrenheit(low_c),
      weather: null // Weather icon parsing would require more work
    });
  }

  // Parse lifts from the facilities table
  // Structure: <td class="lift_name"> with open-icon/close-icon, lift name in <div>, type in next <td>
  const lifts = [];
  const liftSectionMatch = html.match(/Lift Status[\s\S]*?<table[^>]*class="[^"]*facilities_body_table[^"]*"[^>]*>([\s\S]*?)<\/table>/i);
  if (liftSectionMatch) {
    const liftTableHtml = liftSectionMatch[1];
    // Match each lift row: status icon, name, and type
    const liftRowRegex = /<tr[^>]*>[\s\S]*?<td[^>]*class="lift_name"[^>]*>[\s\S]*?<div[^>]*class="lift_open_status_icon"[^>]*>[\s\S]*?<i[^>]*class="[^"]*\b(open-icon|close-icon)\b[^"]*"[^>]*>[\s\S]*?<\/div>[\s\S]*?<div>([^<]+)<\/div>[\s\S]*?<\/td>[\s\S]*?<td[^>]*>([^<]*)<\/td>[\s\S]*?<\/tr>/gi;
    while ((match = liftRowRegex.exec(liftTableHtml)) !== null) {
      const statusClass = match[1];
      const name = match[2].trim();
      const liftType = match[3].trim();
      lifts.push({
        Name: name,
        Status: statusClass === 'open-icon' ? 'Open' : 'Closed',
        IsOpen: statusClass === 'open-icon',
        Type: liftType || (name.includes('Carpet') || name.includes('Conveyor') ? 'Magic Carpet' : 'Chair')
      });
    }
  }

  // Parse trails/runs from the facilities table
  // Structure: <td class="trail_name"> with open-icon/close-icon, run name, grooming icon, difficulty
  const trails = [];
  const trailSectionMatch = html.match(/Run Status[\s\S]*?<table[^>]*class="[^"]*facilities_body_table[^"]*"[^>]*>([\s\S]*?)<\/table>/i);
  if (trailSectionMatch) {
    const trailTableHtml = trailSectionMatch[1];
    // Match each trail row - extract the whole row first, then parse details
    const trailRowRegex = /<tr[^>]*>([\s\S]*?)<\/tr>/gi;
    while ((match = trailRowRegex.exec(trailTableHtml)) !== null) {
      const rowHtml = match[1];

      // Skip header rows
      if (rowHtml.includes('<th')) continue;

      // Check for open/close status icon
      const isOpen = rowHtml.includes('open-icon');
      const isClosed = rowHtml.includes('close-icon');

      // Extract trail name - look for div after trail_open_status_icon div
      const nameMatch = rowHtml.match(/class="trail_name"[\s\S]*?<div[^>]*class="trail_open_status_icon"[^>]*>[\s\S]*?<\/div>[\s\S]*?<div>([^<]+)<\/div>/i);
      if (!nameMatch) continue;
      const name = nameMatch[1].trim();

      // Check for grooming icon
      const isGroomed = rowHtml.includes('icons-snow-plow-truck.svg');

      // Extract difficulty from last td
      const difficultyMatch = rowHtml.match(/<td[^>]*>([^<]*)<\/td>\s*$/i);
      const difficulty = difficultyMatch ? difficultyMatch[1].trim().toLowerCase() : 'unknown';
  ...
  // Get summary counts as fallback
  const runSummaryMatch = html.match(/Run Status[\s\S]*?\((\d+)\s*[\/&;nbsp]+\s*(\d+)\)/i);
  const liftSummaryMatch = html.match(/Lift Status[\s\S]*?\((\d+)\s*[\/&;nbsp]+\s*(\d+)\)/i);
  ...
  return {
    ...
    // Stats - use parsed data, fall back to summary counts
    stats: {
      liftsOpen: lifts.filter(l => l.IsOpen).length || (liftSummaryMatch ? toNumber(liftSummaryMatch[1]) : 0),
      liftsTotal: lifts.length || (liftSummaryMatch ? toNumber(liftSummaryMatch[2]) : 0),
      trailsOpen: trails.filter(t => t.IsOpen).length || (runSummaryMatch ? toNumber(runSummaryMatch[1]) : 0),
      trailsTotal: trails.length || (runSummaryMatch ? toNumber(runSummaryMatch[2]) : 0),
      trailsGroomed: trails.filter(t => t.IsGroomed).length
    },

    // Forecast (if we found any weather data)
...
async function scrapeSunshineVillagePuppeteer(browser) {
  ...
  return {
    ...
    // Use intermaps forecast data (skibanff.com doesn't have forecast)
    forecast: intermapsData.forecast && intermapsData.forecast.length > 0 ? intermapsData.forecast : null
  };
}
...
function toSnowReport(data, resortKey, resortName, localDate) {
  ...
  return {
    ...
    // Convert to Vail-compatible forecast format for app compatibility
    forecast: convertToVailForecastFormat(data.forecast, data.conditions)
  };
}

// Convert Canadian Big3 forecast to Vail-compatible format
function convertToVailForecastFormat(forecast, conditions) {
  if (!forecast || forecast.length === 0) {
    // Even without forecast, provide current temp in the expected structure
    if (conditions?.temperature_c !== null && conditions?.temperature_c !== undefined) {
      return {
        locations: [{
          name: 'Base',
          elevation: null,
          today: {
            high_f: conditions.temperature_f,
            high_c: conditions.temperature_c,
            low_f: conditions.temperature_f,
            low_c: conditions.temperature_c,
            description: null,
  ...
  // Find today's forecast (first entry or one labeled "Today")
  const todayForecast = forecast.find(f => /today/i.test(f.day)) || forecast[0];
  ...
  return {
    locations: [{
      ...
      elevation: null,
      today: {
        high_f: todayForecast?.high_f ?? conditions?.temperature_f,
        high_c: todayForecast?.high_c ?? conditions?.temperature_c,
        low_f: todayForecast?.low_f ?? conditions?.temperature_f,
        low_c: todayForecast?.low_c ?? conditions?.temperature_c,
        description: todayForecast?.weather || null,
        snowfall_day_inches: 0,
        snowfall_night_inches: 0
      },
      forecast_days: forecast.map((day, i) => ({
        date: new Date(Date.now() + i * 86400000).toISOString(),
        high_f: day.high_f,
        high_c: day.high_c,
        low_f: day.low_f,
        low_c: day.low_c,
        description: day.weather || null,
        snowfall_day_inches: 0,
        snowfall_night_inches: 0
      }))
    }]
