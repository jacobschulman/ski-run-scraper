...
function renderTemplate(template, rawData, computedInsights, options = {}) {
  ...
  // Clean up any remaining artifacts from null values
  // Remove phrases like "high of --°" or "--° and Clear" that look broken
  rendered = rendered.replace(/--°F?/g, 'temps TBD');
  rendered = rendered.replace(/high of temps TBD/g, 'temperatures TBD');
  rendered = rendered.replace(/temps TBD and /g, '');
  rendered = rendered.replace(/ with temps TBD/g, '');
