...
function selectHeadlineVariation(condition, resortKey, date, history) {
  ...
  // Check for weekend-specific headlines (use ~65% of the time on weekends)
...
function generateMorningBrief(rawData, computedInsights, options = {}) {
  ...
  // Render body fragments and deduplicate
  const renderedBodies = bodyFragments
    .map(template => renderTemplate(template, rawData, computedInsights, options))
    .filter(text => text && text.trim().length > 0);

  // Remove duplicate sentences (exact match)
  const uniqueBodies = [...new Set(renderedBodies)];

  // Additional deduplication: remove sentences that are substrings of others
  const finalBodies = uniqueBodies.filter((text, index) => {
    return !uniqueBodies.some((other, otherIndex) =>
      otherIndex !== index && other.includes(text) && other.length > text.length
    );
  });

  const bodyText = finalBodies.join(' ');
...
function selectBodyFragmentsWithVariations(firedRules, resortKey, date, history, maxFragments = 3) {
  ...
  // Check if we have a powder day - if so, skip forecast body fragments
  // to avoid contradictory "conditions improving soon" when conditions are already great
  const powderDayRules = ['snow_powder', 'snow_deep', 'snow_epic', 'snow_historic'];
  const hasPowderDay = candidateRules.some(r => powderDayRules.includes(r.id));
  const forecastRulesToSkip = ['forecast_major_storm', 'forecast_storm', 'forecast_snow_coming'];
  ...
  for (const category of categoryOrder) {
    if (byCategory[category]) {
      for (const rule of byCategory[category]) {
        ...
        // Skip forecast body fragments on powder days to avoid contradictory messaging
        if (hasPowderDay && forecastRulesToSkip.includes(rule.id)) continue;
...
function renderTemplate(template, rawData, computedInsights, options = {}) {
  ...
  const tempHighRaw = rawData.forecast?.today?.high_f;
  const tempLowRaw = rawData.forecast?.today?.low_f;
  const tempHigh = (tempHighRaw !== null && tempHighRaw !== undefined) ? tempHighRaw : '--';
  const tempLow = (tempLowRaw !== null && tempLowRaw !== undefined) ? tempLowRaw : '--';

  // Validate lift wait data (filter out obviously bad data like >120 min avg)
  const avgWaitRaw = rawData.lifts?.yesterday?.avgWaitTime || 0;
  const maxWaitRaw = rawData.lifts?.yesterday?.maxWaitTime || 0;
  const avgWait = avgWaitRaw > 120 ? 0 : avgWaitRaw; // Filter bad data
  const maxWait = maxWaitRaw > 180 ? 0 : maxWaitRaw; // Filter bad data
  ...
  const tokens = {
    ...
    // Lifts (with data quality filtering)
    avgWait: avgWait,
    maxWait: maxWait,
    ...
    tempHigh: tempHigh,
    tempLow: tempLow,
    weatherToday: rawData.forecast?.today?.description || 'Variable conditions',
    weatherTomorrow: rawData.forecast?.tomorrow?.description || 'Check forecast',
  ...
  // Clean up any remaining artifacts from null/missing values
  // Catches: "null°", "--°", "high of null°", "with a high of", etc.
  rendered = rendered.replace(/\bnull°F?\b/gi, '');
  rendered = rendered.replace(/--°F?/g, '');
  rendered = rendered.replace(/with a high of\s*°?F?\.?/gi, '');
  rendered = rendered.replace(/high of\s*°?F?\.?/gi, '');
  rendered = rendered.replace(/with temps around\.?/gi, '');
  rendered = rendered.replace(/,?\s*with a\.?\s*/gi, ' ');
  rendered = rendered.replace(/\band\s+\./g, '.');
