...
const rules = [
  ...
  // Epic snow week: 50"+ in 7 days - best week of the season
  {
    id: 'snow_epic_week',
    category: 'snow',
    priority: 93,
    ...
    headlineCondition: 'snow_epic_week',
    bodyFragmentTemplate: 'Incredible timing: {snow7day}" in just 7 days. This is the week you tell stories about.',
    fragmentOrder: 1
  },
  ...
  // Week total - can now generate headline to beat fallback
  {
    id: 'snow_week_total',
    category: 'snow',
    priority: 40,
    ...
    headlineCondition: 'snow_week_total',
  ...
  // NOTE: Priority lowered to 88 so current snow conditions (powder_alert=90)
  // beat forecast when both are true. Today's powder > tomorrow's forecast.
  {
  ...
  // Mellow lift lines
  // NOTE: Suppressed during holidays/busy periods - "mellow" is misleading when crowds expected
  {
    id: 'lift_mellow',
    category: 'lifts',
    priority: 50,
    condition: (rawData, insights, context) => {
      if (!isValidLiftData(rawData)) return false;
      const avgWait = rawData.lifts?.yesterday?.avgWaitTime || 0;
      // Don't claim "mellow" during holidays - crowds are expected regardless of yesterday
      if (isBusyPeriod(context?.date)) return false;
      return avgWait > 0 && avgWait < 4;
    },
    headlineCondition: null,
    bodyFragmentTemplate: 'Lift lines were short yesterday. Similar conditions expected today.',
    fragmentOrder: 2
  },
