...
const rules = [
...

  {
    ...
    condition: (rawData, insights, context) => {
      const dayOfWeek = context?.dayOfWeek || '';
      return ['Tuesday', 'Wednesday', 'Thursday'].includes(dayOfWeek);
    },
...

  {
    ...
    priority: 5,
    condition: (rawData, insights) => {
      // Only fire when nothing notable is happening
      const snow24h = rawData.snow?.['24hour_inches'] || 0;
      const snow7day = rawData.snow?.['7day_inches'] || 0;
    ...
    headlineCondition: null,
    bodyFragmentTemplate: '{openTrails} of {totalTrails} trails are open with {groomedTrails} groomed. Current conditions: {weatherToday} with a high of {tempHigh}°F.',
    fragmentOrder: 99
  }
];
