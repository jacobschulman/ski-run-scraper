...
function shouldScrapeResort(resort, dataType, config, seasonUtils) {
  ...
  if (dataType === 'terrain') {
    ...
    if (checks.inSeason && checks.hasUrl && currentHour >= 5 && currentHour < 11) {
      return true;
    }
    ...
    return forceScrape || (checks.inSeason && checks.hasUrl && !hasBeenScraped);
  }
 ...
 * @param {string} resortKey - The resort identifier
 * @param {object} resortConfig - (Optional) The resort config object for applying overrides
 */
function computeDataCapabilities(resortKey, resortConfig) {
  const capabilities = {
  ...
  // Check if resort is configured to have wait times (from config.json)
  // This is the authoritative source - we trust the config over file existence
  capabilities.liftWaitTimesAvailable = resortConfig?.liftWaitTimesAvailable === true;

  // Apply capability overrides from resort config
  if (resortConfig && resortConfig.dataCapabilityOverrides) {
    Object.assign(capabilities, resortConfig.dataCapabilityOverrides);
  }
...
function generateDataIndex(config) {
  ...
  // Include ALL configured resorts, even if they have no data yet
  Object.keys(RESORTS).forEach(resortKey => {
    const terrainDir = path.join(dataDir, resortKey, 'terrain');
    let files = [];

    if (fs.existsSync(terrainDir)) {
      files = fs.readdirSync(terrainDir)
        .filter(f => f.endsWith('.json') && f !== 'index.json' && f !== 'latest.json')
        .sort()
        .reverse(); // Most recent first
    }

    // Always add resort to index, even with no files
    index.resorts[resortKey] = {
      ...
      count: files.length,
      dataCapabilities: computeDataCapabilities(resortKey, RESORTS[resortKey])
    };
  });
