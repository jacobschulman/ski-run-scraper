...
// ═══════════════════════════════════════════════════════════════════════════════
// ASPEN SNOWMASS NORMALIZERS
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Map Aspen difficulty to standard format
 */
function mapAspenDifficulty(difficulty) {
  if (!difficulty) return 'Unknown';
  const diffLower = difficulty.toLowerCase();
  if (diffLower === 'beginner' || diffLower === 'easiest') return 'Beginner';
  if (diffLower === 'intermediate') return 'Intermediate';
  if (diffLower === 'advanced') return 'Advanced';
  if (diffLower === 'expert') return 'Expert';
  return difficulty.charAt(0).toUpperCase() + difficulty.slice(1);
}

/**
 * Normalize Aspen Snowmass trail data
 */
function normalizeAspenTrail(trail, areaName) {
  return {
    Name: trail.name || 'Unknown Trail',
    IsOpen: trail.isOpen || trail.isDayOpen || false,
    Status: (trail.isOpen || trail.isDayOpen) ? 'Open' : 'Closed',
    Difficulty: mapAspenDifficulty(trail.difficulty),
    IsGroomed: trail.isGroomed || false,
    Area: areaName || null,
    StatusIcon: trail.statusIcon || null
  };
}

/**
 * Normalize Aspen Snowmass lift data
 */
function normalizeAspenLift(lift) {
  return {
    Name: lift.liftName || lift.Name || 'Unknown Lift',
    Status: lift.status || lift.Status || 'Unknown',
    Type: formatLiftType(lift.type || lift.Type),
    Hours: lift.hoursOfOperation || lift.Hours || null,
    ElevationGain: lift.elevationGainFeet ? parseInt(String(lift.elevationGainFeet).replace(',', '')) : null,
    RideTime: lift.time ? parseInt(lift.time) : null,
    Area: lift.area || lift.Area || null
  };
}

/**
 * Normalize combined Aspen Snowmass resort data
 * @param {Object} aspenData - Combined data from fetch (liftStatus, snowReport, groomingReport)
 * @param {string} resortKey - Resort key for identification
 * @returns {Object} - Normalized terrain data
 */
function normalizeAspenResort(aspenData, resortKey) {
  const { liftStatus, snowReport, groomingReport } = aspenData;

  // Normalize lifts
  const lifts = (liftStatus?.liftStatuses || []).map(normalizeAspenLift);

  // Normalize trails from grooming data (organized by area)
  const groomingAreas = [];
  const allTrails = [];

  if (groomingReport?.areas) {
    for (const area of groomingReport.areas) {
      const areaTrails = (area.trails || []).map(t => normalizeAspenTrail(t, area.name));
      allTrails.push(...areaTrails);

      groomingAreas.push({
        Name: area.name,
        IsOpen: area.isOpen || false,
        IsGatedTerrain: area.isGatedTerrain || false,
        Trails: areaTrails,
        Lifts: [] // Lifts are separate in Aspen's API
      });
    }
  }
  ...
  return {
    ...
    SnowReport: snowReport ? {
      LastUpdated: snowReport.lastUpdated,
      Status: snowReport.status,
      Snow24h: snowReport.snow24Hours?.inches ? parseInt(snowReport.snow24Hours.inches) : null,
      Snow48h: snowReport.snow48Hours?.inches ? parseInt(snowReport.snow48Hours.inches) : null,
      Snow7Day: snowReport.snow7Days?.inches ? parseInt(snowReport.snow7Days.inches) : null,
      BaseDepth: snowReport.snowBase?.inches ? parseInt(snowReport.snowBase.inches) : null,
      TrailsOpen: snowReport.trails?.openCount,
      TrailsTotal: snowReport.trails?.totalCount,
      LiftsOpen: snowReport.lifts?.openCount,
      LiftsTotal: snowReport.lifts?.totalCount,
      AcresOpen: snowReport.acres?.openCount,
      AcresTotal: snowReport.acres?.totalCount
    } : null,
    NoonGroom: groomingReport?.noonGroom || null,
    Provider: 'aspensnowmass'
  };
}

/**
 * Normalize Aspen Snowmass snow report for snow-scraper compatibility
 */
function normalizeAspenSnowReport(aspenData, resortKey, resortName, localDate) {
  const { snowReport } = aspenData;
  ...
  return {
    ...
    date: localDate,
    provider: 'aspensnowmass',
    status: snowReport.status || 'Unknown',
    lastUpdated: snowReport.lastUpdated || null,
    snow: {
      new24h: snowReport.snow24Hours?.inches ? parseInt(snowReport.snow24Hours.inches) : null,
      new48h: snowReport.snow48Hours?.inches ? parseInt(snowReport.snow48Hours.inches) : null,
      new7day: snowReport.snow7Days?.inches ? parseInt(snowReport.snow7Days.inches) : null,
      baseDepth: snowReport.snowBase?.inches ? parseInt(snowReport.snowBase.inches) : null,
      seasonTotal: null // Not provided by Aspen API
    },
    terrain: {
      trailsOpen: snowReport.trails?.openCount || null,
      trailsTotal: snowReport.trails?.totalCount || null,
      liftsOpen: snowReport.lifts?.openCount || null,
      liftsTotal: snowReport.lifts?.totalCount || null,
      acresOpen: snowReport.acres?.openCount || null,
      acresTotal: snowReport.acres?.totalCount || null
    },
    conditions: {
      primary: snowReport.status || null,
      secondary: null,
      surface: null
    },
...
module.exports = {
  ...
  normalizeAspenTrail,
  normalizeAspenLift,
  normalizeAspenResort,
  normalizeAspenSnowReport,
  mapAspenDifficulty,
