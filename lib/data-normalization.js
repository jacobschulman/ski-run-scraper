...
function mapAspenDifficulty(difficulty) {
  ...
  if (diffLower === 'beginner' || diffLower === 'easiest') return 'Green';
  if (diffLower === 'intermediate') return 'Blue';
  if (diffLower === 'advanced') return 'Black';
  if (diffLower === 'expert') return 'DoubleBlack';
  if (diffLower === 'extreme') return 'DoubleBlack';
  if (diffLower === 'terrain-park' || diffLower === 'park') return 'TerrainPark';
...
function normalizeAspenSnowReport(snowReport, resortKey, resortName, localDate) {
  ...
  // Aspen provides both inches and centimeters
  ...
  return {
    ...
    snowfall: {
      overnight_inches: 0, // Aspen API doesn't provide overnight separate from 24h
      overnight_cm: 0,
      ...
      season_total_inches: 0, // Not provided by Aspen API
      season_total_cm: 0
    },
    ...
    terrain: {
      ...
      groomedTrails: 0, // Not provided in snow report
      totalLifts: snowReport.lifts?.totalCount || 0,
      openLifts: snowReport.lifts?.openCount || 0,
      totalAcres: snowReport.acres?.totalCount || 0,
      openAcres: snowReport.acres?.openCount || 0
    },
