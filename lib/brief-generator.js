...
function calculateInsights(rawData) {
  ...
  if (rawData.terrain) {
    ...
    if (terrain.newlyGroomed.length > 0) {
      insights.alerts.push(`${terrain.newlyGroomed.length} trails freshly groomed`);
    }
    ...
    if (terrain.stats.openTrails.delta > 0) {
      insights.trends.push(`${terrain.stats.openTrails.delta} more trails open than yesterday`);
    }
